/**
 * Endpoint para injetar token QStash no localStorage e continuar instalacao
 * Acesse: GET /api/install/inject-qstash-token?token=eyJ...
 *
 * Injeta o token no estado de instalacao (step 4) e redireciona para /install
...
export async function GET(request: Request) {
  ...
  if (!token) {
    return new NextResponse(
      ...
      <html>
        <head>
          <title>Injetar Token QStash</title>
          <meta charset="utf-8" />
          <style>
            body {
              background: #0a0a1a;
        ...
        <body>
          <div class="container">
            ...
            <div class="error">
              Forneca o token QStash via parametro de URL
            </div>
            <p class="info">Formato:</p>
            <code>
              /api/install/inject-qstash-token?token=eyJ_seu_token_aqui
            </code>
            <p class="info">Copie o QSTASH_TOKEN completo da aba Details no console Upstash</p>
          </div>
        </body>
      </html>`,
      {
  ...
  // Estado com o token QStash injetado no step 4
  ...
  const html = `
    ...
    <html>
      <head>
        <title>Injetando token QStash...</title>
        <meta charset="utf-8" />
        <style>
          body {
            background: #0a0a1a;
